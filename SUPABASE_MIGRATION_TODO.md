...
Phases 1–6 done and verified against a real local Supabase stack (Docker/OrbStack): project scaffolding, real Supabase Auth, Field Guide as the CRUD+RLS vertical slice, the event-cancellation server-push pipeline (verified to the FCM credentials boundary), full Profile completion, and Challenges (accept/toggle-rep/current-challenge-resolution, all RLS/RPC-verified against real data, including the "most recent non-cancelled Frat Night" resolution via real `events`+`event_frat_night_details` rows, not just template dates). All three of ADR 0002's POC success criteria are met at the backend level. 24 widget tests pass, `flutter analyze` clean.

Two more real bugs caught by testing against the real stack (this keeps happening every phase — treat "test the actual write/read path against a live instance" as mandatory, not optional, for whatever's built next):
- Phase 5: a client-direct `INSERT` on `user_reminders`/`user_devices` that omits `user_id` sends `NULL`, and `with check (user_id = auth.uid())` evaluates `NULL = uuid` as `NULL` (not true) — same generic RLS-violation error a real cross-user attempt gets. Fixed with `default auth.uid()` on both tables. **Any future table with a client-direct insert needs this same default.**
- Phase 6: `has_challenge_member_association()` was used in policies before it was ever defined anywhere — planned in earlier design notes but never actually written into a migration. Had to add it to the `challenges` migration itself (not the earlier `members_and_associations` migration where its sibling `has_member_association` lives) since it references `challenge_members`, which doesn't exist until then — a real migration-ordering constraint, not just organization. **Worth double-checking any RPC/policy referencing a `has_..._association`-style helper actually has that helper defined somewhere before assuming it does.**
...
- **Derive household member lists/labels from real `Member` records.** Today/Guide/Events still hardcode `'you'/'jack'/'thomas'` as literal member keys and display labels throughout the UI/provider layer (e.g. `guide_screen.dart`'s `_personLabels` map, `GuideSelectedPerson.build() => 'you'`). **Challenge is partially fixed** — `challengeHouseholdProvider` now derives real `ChallengeHouseholdMember`s from `householdMembersProvider`, using `member.firstName` as the label — but this drops the "You" special-casing the self Member used to get (a Guardian now sees their own first name instead of "You" on their own person-tab). Not fixed, just no longer worse than before; still needs the same treatment across Today/Guide/Events, plus deciding whether "You" comes back for the self Member specifically.
